...
export class UsersListComponent implements AfterViewInit, OnDestroy{
    ...
    constructor(
        ...
        public snackBar: MatSnackBar) {
            this.state = this.stateService.usersListState;
        }
    ...
    create() {
        const user = new User();
        const dlg = this.dialog.open(UserEditComponent, { data: { user, isNew: true } });
        dlg.beforeClosed().subscribe(res => {
            if (res) {
                res = plainToClass(User, res);
    ...
    ngAfterViewInit(): void {
        ...
        this.dataSource.filterPredicate = (data: User, filter: string) => {
            const str = data.pseudo + ' ' + data.email + ' ' + data.firstname + ' ' + data.lastname + ' ' + data.birthdate + ' ' + data.roleToString;
